    ...
    public class RecursionChallenge {
            ...
            public static String reverse(String s){
                if (s.length() <= 1) {
                    return s;
                } else return reverse(s.substring(s.length() / 2)) + reverse(s.substring(0, s.length() / 2));
            }

            public static Boolean isPalindrome(String s){
                if (s.length() <= 1) {
                    return true;
                } else if (s.length() == 2 || !s.substring(0 , 1).equals(s.substring(s.length() - 1))) {
                    return false;
